...
if [[ "$1" == "--help" || "$1" == "-h" ]]; then
    ...
    echo "  7. Configures GitHub DAG synchronization"
    echo "  8. Tests the complete pipeline"
    echo ""
    echo "RESULT:"
    echo "  Complete working data engineering stack with:"
    echo "  • Apache Airflow 2.10.5 with GitHub DAG sync"
    echo "  • SparkOperator with apache/spark:3.5.5"
    echo "  • Jupyter Lab"
    echo "  • LocalStack S3"
    echo "  • Automated DAG deployment from GitHub"
...
echo "• GitHub DAG synchronization from:"
echo "  https://github.com/nickpeachey/airflow-elyra"
...
echo ""
echo -e "${BLUE}Redeploy notebooks only:${NC}"
echo "   $SCRIPT_DIR/deploy-content.sh"
echo ""
echo -e "${BLUE}Check GitHub DAG sync status:${NC}"
echo "   $SCRIPT_DIR/check-git-sync.sh"
echo ""
...
echo "2. Open Airflow UI and wait for DAGs to sync from GitHub (60 sec intervals)"
echo "3. Explore the spark_operator_s3_pipeline DAG once it appears"
echo "4. The pipeline will automatically process sales data and generate analytics"
echo "5. Check the results in LocalStack S3 buckets"
echo "6. Use check-git-sync.sh to monitor DAG synchronization status"
echo ""
