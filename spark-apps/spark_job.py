...
from pyspark.sql.functions import col, sum as spark_sum, avg, count, round as spark_round, max as spark_max, min as spark_min
...
def process_daily_metrics(df, output_path):
    ...
    daily_summary = df.agg(
        ...
        df.select("transaction_date").first()[0].alias("processing_date")
